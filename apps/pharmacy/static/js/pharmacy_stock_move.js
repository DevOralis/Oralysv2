  ...
  document.addEventListener('DOMContentLoaded', () => {    
    ...
    function createProductSelect(selectedId = '') {
      ...
      productsData.forEach(prod => {
        const displayName = prod.short_label || prod.name || 'Produit sans nom';
    ...
    function loadOrdersForSupplier(supplierId, cb) {
      ...
      fetch(`/pharmacy/ajax/supplier-orders/?supplier_id=${supplierId}`)
    ...
    function loadLinesForOrder(orderId) {
      ...
      fetch(`/pharmacy/ajax/order-lines/?order_id=${orderId}`)
        .then(r => (r.ok ? r.json() : Promise.reject(r.status)))
        .then(json => {
    ...
    function toggleFields() {
      ...
      if (isTransfert) {
        // Pour les transferts : afficher postes source et destination
        sourceLocationCol.classList.add('show');
        destLocationCol.classList.add('show');
      } else if (isConsommation) {
        // Pour les consommations : afficher poste source et département
        sourceLocationCol.classList.add('show');
        departmentField.classList.add('show');
      } else if (isReception) {
        // Pour les réceptions : afficher poste destination et fournisseur
        destLocationCol.classList.add('show');
    ...
    moveForm?.addEventListener('submit', e => {
      ...
      const data = Array.from(tbody.querySelectorAll('tr')).map(row => {
        ...
        console.log('=== VALIDATION STOCK PHARMACY ===');
        ...
        if (
          ...
          if (stockDispo === null || stockDispo === undefined) {
            console.log('❌ Produit absent du poste source');
            hasError = true;
      ...
      if (validProductsCount === 0) {
        Swal.fire({
          icon: 'error',
          title: 'Produit ou quantité manquante',
        ...
        switch (error.error) {
          case 'missing_data':
            Swal.fire({
              ...
              text: 'Veuillez sélectionner un produit et saisir une quantité demandée pour chaque ligne.',
              confirmButtonColor: '#dc3545',
              confirmButtonText: 'OK'
            });
            break;
          case 'duplicate':
            Swal.fire({
              ...
              text: 'Un même produit ne peut pas être sélectionné plusieurs fois dans le même mouvement.',
              confirmButtonColor: '#dc3545',
              confirmButtonText: 'OK'
            });
            break;
          case 'absent':
            Swal.fire({
              icon: 'error',
              title: 'Produit absent du poste source',
              text: "Ce produit n'existe pas dans le poste de stockage source sélectionné.",
              confirmButtonColor: '#dc3545',
              confirmButtonText: 'OK'
            });
            break;
          case 'qty_demanded':
            Swal.fire({
              ...
              text: `La quantité demandée (${error.quantity_demanded}) dépasse le stock disponible (${error.stockDispo}) dans le poste source.`,
              confirmButtonColor: '#dc3545',
              confirmButtonText: 'OK'
            });
            break;
          case 'qty_arrived':
            Swal.fire({
              icon: 'warning',
              title: 'Stock insuffisant',
              text: `La quantité réellement transférée (${error.quantity_arrived}) dépasse le stock disponible (${error.stockDispo}) dans le poste source.`,
              confirmButtonColor: '#dc3545',
              confirmButtonText: 'OK'
            });
            break;
      ...
      // Vérif postes identiques (transfert)
      if (!isConsommation && !isReception && sourceField && destField &&
          sourceField.value && destField.value && sourceField.value === destField.value) {
        hasError = true;
          Swal.fire({
            icon: 'warning',
            title: 'Postes de stockage identiques',
          text : "Le poste de destination doit être différent du poste source.",
            confirmButtonColor: '#dc3545',
            confirmButtonText: 'OK'
          });
      ...
      if (!isConsommation && destField && !destField.value) {
        ...
        Swal.fire({
          icon: 'warning',
          title: 'Poste de stockage manquant',
          text : 'Veuillez sélectionner un poste de stockage de destination.',
          confirmButtonColor: '#dc3545',
          confirmButtonText: 'OK'
        });
        destField.classList.add('is-invalid');
